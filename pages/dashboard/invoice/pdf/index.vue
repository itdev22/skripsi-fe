    ...
    >
      <div class="flex justify-between items-start mb-6">
        <div>
          ...
          <p class="text-sm text-gray-500">{{ invoiceNumber }}</p>
          <div class="mt-2">
          ...
          <div class="mt-4 text-sm">
            ...
            <p>ATTN: {{ props.invoice.customer.attention }}</p>
            <p>{{ props.invoice.customer.address }}</p>
            <p>
              {{ props.invoice.customer.city }},
              {{ props.invoice.customer.province }} -
              {{ props.invoice.customer.postalCode }}
            </p>
            <p>{{ props.invoice.customer.country }}</p>
            <p>Phone: {{ props.invoice.customer.phone }}</p>
            <p>Email: {{ props.invoice.customer.email }}</p>
          </div>
        </div>
        <div class="text-right text-sm">
          ...
          <p class="mt-4">Invoice Date: {{ props.invoice.date }}</p>
          <p>Due Date: {{ props.invoice.dueDate }}</p>
          <p class="mt-4 font-bold">
            Invoice Total: Rp {{ props.invoice.total.toLocaleString("id-ID") }}
          </p>
          <p>
            Total Paid: Rp {{ props.invoice.totalPaid.toLocaleString("id-ID") }}
          </p>
          <p>
            Amount Due: Rp {{ props.invoice.amountDue.toLocaleString("id-ID") }}
          </p>
        </div>
      </div>
      ...
      <table class="w-full text-sm mb-6 border-t border-b border-gray-300">
        ...
        <tbody>
          <tr v-for="(item, i) in props.invoice.items" :key="i">
            <td class="py-2">{{ i + 1 }}</td>
            <td class="py-2">{{ item.name }}</td>
            <td class="py-2">Rp {{ item.price.toLocaleString("id-ID") }}</td>
            <td class="py-2">{{ item.quantity }}</td>
            <td class="py-2">Rp {{ item.total.toLocaleString("id-ID") }}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="flex justify-end mb-6">
        <div class="text-sm w-1/3">
          <div class="flex justify-between">
            <span>Sub Total</span>
            <span>Rp {{ props.invoice.subtotal.toLocaleString("id-ID") }}</span>
          </div>
          <div class="flex justify-between">
            <span>TAX</span>
